...
codetree <- GetCodeTree("trade", "ct_raw_tf", dimension = itemVar)

cerealItemCode <- adjacent2edge(codetree)[parent == "10", children]

dms <- list(Dimension(name = "reportingCountryM49",
                      keys = "484"),
...
k <- DatasetKey(domain     = "trade",
                dataset    = "ct_raw_tf",
                dimensions = dms)

d <- GetData(key = k,
             normalized = FALSE, 
             pivoting = c(Pivoting(code = "reportingCountryM49"),
                          Pivoting(code = "partnerCountryM49"),
                          Pivoting(code = "measuredItemHS"),
                          Pivoting(code = "timePointYears"),
                          Pivoting(code = "measuredElementTrade")
                          ))
